...
int main(int argc, char **argv) {
  ...
  if (argc != 4) {
    printf("Incorrect number of arguments.\nUsage: ./hw2_ex2.out <numARows> <numAColumns> <numBColumns>\n");
    return -1;
  }
  ...
  for (int i = 0; i < numARows * numAColumns; ++i) {
    hostA[i] = ((DataType)rand() / (DataType)RAND_MAX) * MAXIMUM_RANDOM_VALUE;
  }

  for (int i = 0; i < numBRows * numBColumns; ++i) {
    hostB[i] = ((DataType)rand() / (DataType)RAND_MAX) * MAXIMUM_RANDOM_VALUE;
  }
  ...
  for (int y = 0; y < numCRows; ++y) {
    ...
    for (int x = 0; x < numCColumns; ++x) {
      ...
      for (int i = 0; i < numAColumns; ++i) {
        resultRef[x + y * numCColumns] += hostA[i + y * numAColumns] * hostB[x + i * numBColumns];
      }
    }
  }
